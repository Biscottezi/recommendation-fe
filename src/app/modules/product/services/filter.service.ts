...
export class FilterService {
  ...
  filterList = new BehaviorSubject<Category[]>([]);
  products!: Product[];
  category = '';
  ...
  handleCatFilter(checkedItems:Category[]): Product[]{
  ...
  handleRateFilter(rating: number): Product[] {
    ...
    // this.filterProduct(this.products);
    // this.filteredProducts.subscribe((data: Product[]) => {
    //  this.products = data.filter(item => rating <= Math.trunc(item.average_rating));
    // })
    return this.products;
  }
